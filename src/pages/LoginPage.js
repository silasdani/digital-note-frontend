...
const RightOverlayContent = ({ isAnimated, setIsAnimated }) => {
  return (
    <div className="p-8 text-center">
      <h1 className="text-6xl font-bold text-white mb-4">Do not have an account ?</h1>
  ...
  return (
    <div className="flex flex-col items-center h-full w-full">
      <div className="min-h-[85vh] mt-10 w-4/5 bg-white relative overflow-hidden rounded-lg">
        ...
        <div
          className={`overlay-container absolute top-0 left-1/2 w-1/2 h-full overflow-hidden  transition-transform duration-700 ease-in-out z-100 ${
            isAnimated ? '-translate-x-full' : ''
          }`}
        >
          <div
            className={`overlay ${overlayBg} relative -left-full h-full w-[200%] transform  transition-transform duration-700 ease-in-out ${
              isAnimated ? 'translate-x-1/2' : 'translate-x-0'
            }`}
          >
            <div
              className={`overlay-left w-1/2 h-full absolute flex justify-center items-center top-0 transform -translate-x-[0%]  transition-transform duration-700 ease-in-out ${
            ...
            <div
              className={`overlay-right w-1/2 h-full absolute flex justify-center items-center top-0 right-0 transform  transition-transform duration-700 ease-in-out ${
