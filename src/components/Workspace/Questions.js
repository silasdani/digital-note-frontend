...
const Questions = (props) => {
  ...
  return (
    <div>
      ...
      {questionType == 'file' && (
        <div className="flex flex-col items-center my-10 px-10 pb-5 border rounded-xl cursor-pointer">
          {!currentAnswer?.file && (
            <>
              <div
                className="flex flex-col items-center px-10 pb-5 border rounded-xl cursor-pointer"
          ...
          <div className="file-section justify-center">
            {currentAnswer?.file && (
              <div className="flex flex-col items-center">
                {isImage(currentAnswer?.file) && (
                  <img className="my-5 rounded-xl" src={currentAnswer?.file} alt="Image" />
                )}
          ...
          {selects?.map((select, selectIndex) => (
            ...
            >
              <input
                ...
                checked={currentAnswer?.selects?.includes(select)}
                onChange={() => {
                  props.updateQuestionFields(
                    currentAnswer?.no,
                    'selects',
                    xor(currentAnswer?.selects, [select])
                  );
                }}
              />
      ...
      <div className="my-10 mx-auto w-56 btn-group grid grid-cols-2">
        {questions[0].no < currentQuestion.no ? (
          <button className="btn btn-outline" onClick={onPrevQuestion}>
            Previous
          </button>
        ) : (
          <div />
        )}
        {questions[questions.length - 1].no > currentQuestion.no && (
          <button className="btn btn-outline" onClick={onNextQuestion}>
            Next
          </button>
        )}
        {questions[questions.length - 1].no == currentQuestion.no && (
          <button className="btn" onClick={props.onNextTab}>
            Next
          </button>
        )}
      </div>
