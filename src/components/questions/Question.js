      ...
      <div className="flex flex-col-reverse lg:flex-row lg:space-x-5 lg:justify-between">
        ...
        <div className="flex flex-row space-x-5 justify-between items-center min-w-[20vw] lg:items-end lg:flex-col lg:space-y-2 lg:space-x-0  lg:w-[38%]">
          ...
          <div className="flex space-x-5">
            ...
            {!viewMode && (
              <button className="btn btn-outline" onClick={() => questionFileRef.current?.click()}>
                + ADD FILE
              </button>
            )}
            <input
          ...
          <div className="flex flex-col items-center">
            ...
            {!viewMode && (
              <button
                ...
                onClick={() => {
                  props.updateQuestionFields(index, 'file', null);
                }}
              >
                Remove File
              </button>
      ...
      <div className="question-content-section py-2">
          ...
          <div className="">
            <div>
              <input
                ...
                placeholder="Text answer goes here"
              />
            </div>
          </div>
        )}
        {questionType == 'option' && (
          <div className="flex flex-col lg:flex-row lg:space-x-6">
            ...
            {!viewMode && (
              <button
                onClick={() => props.updateQuestionFields(index, 'options', [...options, 'new'])}
              >
                <svg
                  className="rounded-full bg-accent h-8 w-8"
                  fill="#fff"
                  viewBox="0 0 20 20"
                  xmlns="http://www.w3.org/2000/svg"
                >
        ...
        {questionType == 'file' && (
          <div className="flex flex-col items-center px-10 pb-5 border rounded-xl cursor-pointer">
            <div className="">
        ...
        {questionType == 'choose' && (
          <div className="flex flex-col lg:flex-row lg:space-x-6">
                      ...
                      <button
                        onClick={() =>
                          props.updateQuestionFields(
                            index,
                            'selects',
                            selects.filter((_, i) => i != selectIndex)
                          )
                        }
                      >
                        <svg
            ...
            {!viewMode && (
              <button
                onClick={() =>
                  props.updateQuestionFields(index, 'selects', [...selects, 'new select'])
                }
              >
                <svg
                  className="rounded-full bg-accent h-8 w-8"
      ...
      {!viewMode && (
        <>
          <div className="flex justify-start space-x-1">
            <button className="btn btn-accent btn-sm" onClick={() => props.addNewQuestionAt(no)}>
              Paste Next
            </button>
            {canDelete && (
              <button className="btn btn-error btn-sm" onClick={() => props.removeQuestion(index)}>
                Remove
              </button>
            )}
          </div>
